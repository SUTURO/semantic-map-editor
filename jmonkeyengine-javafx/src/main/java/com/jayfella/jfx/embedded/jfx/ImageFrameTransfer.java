...
public class ImageFrameTransfer extends AbstractFrameTransfer<ImageView> {
    ...
    public ImageFrameTransfer(ImageView imageView, FrameTransferSceneProcessor.TransferMode transferMode, FrameBuffer frameBuffer, int width, int height) {
        super(imageView, transferMode, frameBuffer, width, height);
        Platform.runLater(() -> imageView.setImage(writableImage));
    }

    @Override
    protected PixelWriter getPixelWriter(ImageView destination, FrameBuffer frameBuffer, int width, int height) {
